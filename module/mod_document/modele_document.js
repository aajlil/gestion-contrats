...
exports.getDocumentsUtilisateur = async (utilisateur_id) => {
    const result = await pool.query(
        "SELECT d.id_document, d.nom_fichier, d.chemin, d.type, d.uploader_a, d.contrat_id, c.nom AS contrat_nom " +
        "FROM document d " +
...
exports.getDocumentUtilisateurById = async (id_document, utilisateur_id) => {
    const result = await pool.query(
        "SELECT d.id_document, d.nom_fichier, d.chemin, d.type, d.uploader_a, d.contrat_id " +
        "FROM document d " +
        "LEFT JOIN contrat c ON d.contrat_id = c.id_contrat " +
